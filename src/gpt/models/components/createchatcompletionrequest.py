...
from .chatcompletionrequestmessage import AssistantMessage, FunctionMessage, SystemMessage, ToolMessage, UserMessage
from .chatcompletiontool import ChatCompletionTool
from .chatcompletiontoolchoiceoption import ChatCompletionNamedToolChoiceSchemas, ChatCompletionToolChoiceOption1
...
class CreateChatCompletionRequest:
    messages: List[Union[SystemMessage, UserMessage, AssistantMessage, ToolMessage, FunctionMessage]] = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('messages') }})
    ...
    """
    function_call: Optional[Union[One, Schemas]] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('function_call'), 'exclude': lambda f: f is None }})
    ...
    **Important:** when using JSON mode, you **must** also instruct the model to produce JSON yourself via a system or user message. Without this, the model may generate an unending stream of whitespace until the generation reaches the token limit, resulting in a long-running and seemingly \"stuck\" request. Also note that the message content may be partially cut off if `finish_reason=\"length\"`, which indicates the generation exceeded `max_tokens` or the conversation exceeded the max context length.
    ...
    """
    tool_choice: Optional[Union[ChatCompletionToolChoiceOption1, ChatCompletionNamedToolChoiceSchemas]] = dataclasses.field(default=None, metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('tool_choice'), 'exclude': lambda f: f is None }})
